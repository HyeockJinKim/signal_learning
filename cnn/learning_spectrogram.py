...
class SpectrogramLearning:
    ...
    def get_accuracy_percent(self, logits, targets):
        temp = np.argpartition(logits, -2)[-2:]

        batch_predictions = np.argmax(logits, axis=1)
        for t in temp:
            self.make_confusion_matrix(t, targets)
    ...
    def learning_cnn(self, lr, generations, drop_out_rate, batch_size):
        ...
        with tf.Session(config=config) as sess:
            ...
            # my_optimizer2 = tf.train.AdamOptimizer(learning_rate=lr,)  #  epsilon=10e-2)
    ...
    def save_result(self, memo=None):
        save_path = os.path.join('cnn', 'save')
        with open(os.path.join(save_path, 'save_'+str(len(os.listdir(save_path)))), 'w') as f:
